var y="https://sebe-verify-sdk.vercel.app";function m(i,e){let t;try{t=new URL(i)}catch{throw new Error(`${e} must be an absolute http(s) URL, got "${i}"`)}if(t.protocol!=="http:"&&t.protocol!=="https:")throw new Error(`${e} must be an http(s) URL, got "${t.protocol}"`)}function x(){if(typeof crypto<"u"&&typeof crypto.randomUUID=="function")return crypto.randomUUID();let i=new Uint8Array(16);if(typeof crypto<"u"&&typeof crypto.getRandomValues=="function")crypto.getRandomValues(i);else for(let t=0;t<16;t++)i[t]=Math.floor(Math.random()*256);i[6]=i[6]&15|64,i[8]=i[8]&63|128;let e=Array.from(i,t=>t.toString(16).padStart(2,"0")).join("");return`${e.slice(0,8)}-${e.slice(8,12)}-${e.slice(12,16)}-${e.slice(16,20)}-${e.slice(20)}`}var g=class{config;eventListeners=new Map;sessionId=null;modalElement=null;webAppUrl;constructor(e){if(!e.apiKey)throw new Error("apiKey is required");if(!e.projectId)throw new Error("projectId is required");if(!e.redirectUrl)throw new Error("redirectUrl is required");m(e.redirectUrl,"redirectUrl");let t=e.webAppUrl||y;m(t,"webAppUrl"),this.config=e,this.webAppUrl=t.replace(/\/$/,"")}on(e,t){return this.eventListeners.has(e)||this.eventListeners.set(e,[]),this.eventListeners.get(e).push(t),this}off(e,t){let n=this.eventListeners.get(e);if(n){let r=n.indexOf(t);r>-1&&n.splice(r,1)}return this}emit(e,t){(this.eventListeners.get(e)||[]).forEach(r=>{try{r(t)}catch(d){console.error(`Error in ${e} handler:`,d)}})}buildVerificationUrl(e){let t=new URLSearchParams({returnUrl:this.config.redirectUrl,projectId:this.config.projectId,apiKey:this.config.apiKey});return`${this.webAppUrl}/verify/${e}?${t.toString()}`}createModal(e){if(this.modalElement)return;if(!document.getElementById("sv-geist-font")){let l=document.createElement("link");l.id="sv-geist-font",l.rel="stylesheet",l.href="https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600;700&family=Geist+Mono:wght@500&display=swap",document.head.appendChild(l)}let t="'Geist', ui-sans-serif, system-ui, -apple-system, sans-serif",n=document.createElement("div");n.setAttribute("role","dialog"),n.setAttribute("aria-modal","true"),n.setAttribute("aria-labelledby","sebeverify-modal-title"),n.style.cssText=`
    ...
    `,c.textContent="Verify your identity to continue. You'll need your ID document and a selfie \u2014 takes about 2 minutes.";let p=document.createElement("div");p.style.cssText=`
    ...
    `,o.addEventListener("mouseover",()=>{o.style.color="#4B5063"}),o.addEventListener("mouseout",()=>{o.style.color="#8B8F9C"});let f=document.createElement("div");f.style.cssText=`
    ...
    `,r.append(d,s,c,p,a,o,f),n.appendChild(r);let u=()=>{this.closeModal(),this.emit("cancelled")};o.addEventListener("click",u),n.addEventListener("click",l=>{l.target===n&&u()});let h=l=>{l.key==="Escape"&&u()};document.addEventListener("keydown",h),n.__cleanup=()=>{document.removeEventListener("keydown",h)},document.body.appendChild(n),this.modalElement=n}closeModal(){if(!this.modalElement)return;let e=this.modalElement.__cleanup;e&&e(),this.modalElement.remove(),this.modalElement=null}isMobile(){return typeof window>"u"?!1:/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}async start(){try{this.emit("started");let e=x();if(this.sessionId=e,this.config.registerSessionUrl){let n=await fetch(this.config.registerSessionUrl,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({sessionId:e})});if(!n.ok){let r=await n.json().catch(()=>({}));throw new Error(r.error??`Session registration failed (${n.status})`)}}let t=this.buildVerificationUrl(e);if(this.isMobile()){window.location.href=t,this.emit("mobile_opened");return}this.createModal(t)}catch(e){this.closeModal();let t=e instanceof Error?e.message:"Unknown error";throw this.emit("error",new Error(t)),e}}destroy(){this.closeModal(),this.eventListeners.clear(),this.sessionId=null}};function b(i){return new g(i)}async function v(i){let e=i.documentType||"national-id",t=i.documentId||`user_${Date.now()}_${Math.random().toString(36).slice(2,11)}`,n=`${i.backendUrl}/projects/${i.projectId}/verification/session/start`,r=await fetch(n,{method:"POST",headers:{"Content-Type":"application/json","X-API-Key":i.apiKey},body:JSON.stringify({document_type:e,document_id:t})});if(!r.ok){let s=await r.json().catch(()=>({detail:"Failed to create verification session"})),c=typeof s?.detail=="string"?s.detail:JSON.stringify(s?.detail??s);throw new Error(`${c||"Failed to create verification session"} (${r.status})`)}return{sessionId:(await r.json()).session_id,backendUrl:i.backendUrl,projectId:i.projectId}}export{g as SebeVerifySDK,v as createVerificationSession,b as default};
